...
function DashboardNavbar({ absolute, light, isMini }) {
  ...
  const settingsRendering = () => (
    <Menu
      ...
      sx={{ mt: 2 }}
    >
      <SettingItem icon={<Icon>account_circle</Icon>} title="Profile" />
      <SettingItem icon={<Icon>settings</Icon>} title="Settings" />
      <SettingItem icon={<Icon>logout</Icon>} title="LogOut" onClick={logout} />
    </Menu>
  );
  ...
  const logout = async () => {
    ...
    const response = await fetch(Url.api + Url.userLogOut, {
      method: "GET",
      headers: {
    ...
    try {
      ...
      console.log("fetch error from logout-----" + error);
    }
  };
