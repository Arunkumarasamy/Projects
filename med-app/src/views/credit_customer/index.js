...
import { List_credit } from "./components/list_credit_customer_data";
import Add_New_Credit from "./components/add_new_credit";
import styled from "@emotion/styled";
import { Close, HighlightOff } from "@mui/icons-material";
import Add_Credit from "./components/add_credit";
import View_cus_credits from "./components/view_customer_credit";

export default function Credit_Customer() {
  ...
  const creditList = async (e) => {
    ...
    try {
      ...
      const response = await fetch(Url.api + Url.creditList, {
        method: "POST",
        headers: {
      ...
      if (responceData.apiStatus.code == "200") {
        setList(responceData.result.customerData);
  ...
  useEffect(() => {
    const { columns: newColumns, rows: newRows } = List_credit(
      list,
      openView,
      openAdd
    );
